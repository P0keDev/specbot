...
public class DCommandHandler {
	...
	private static HashMap<String, DCommandBase> commandRegistry = new HashMap<String, DCommandBase>();
	private static List<String> cNames = new ArrayList<String>();

	public DCommandHandler() {
		File commandList = new File("./discordcommands.txt");
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				DCommandBase ctr = (DCommandBase) Class.forName("p0ke.specbot.command.discord." + parts[parts.length - 1])
			...
			System.out.println("Registered " + commandRegistry.size() + " discord commands!");
			reader.close();
