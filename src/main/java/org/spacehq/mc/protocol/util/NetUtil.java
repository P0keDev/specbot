...
public class NetUtil {
    ...
    public static CompoundTag readNBT(NetInput in) throws IOException {
        ...
        if(b == 0) {
            ...
            return (CompoundTag) NBTIO.readTag((InputStream) new DataInputStream(new NetInputStream(in, b)));
        }
    }
    ...
    public static void writeNBT(NetOutput out, CompoundTag tag) throws IOException {
        if(tag == null) {
            ...
            NBTIO.writeTag((OutputStream) new DataOutputStream(new NetOutputStream(out)), tag);
        }
    }
